...
description: Delegate implementation to Codex CLI with structured planning, execution, and iterative quality review
user_invocable: true
---

# Codex Implementation

Delegate implementation to Codex CLI. Claude creates a detailed plan, Codex executes it, then the result is reviewed and refined until quality passes.

## Key Principle

**Claude plans, Codex executes.** Claude's strength is analyzing context and designing solutions. Codex's strength is executing precise instructions at scale. The plan file is the contract between them.

Codex runs in the **same working directory** as this project. It can read any files and modify them directly. The plan only needs to specify what to implement — Codex handles the file operations.
...
```bash
PLAN_FILE=$(mktemp /tmp/codex-impl-plan-XXXXXX.md)
```
   ...
   **The plan must be specific enough that Codex can execute without guessing.** Include exact file paths, function signatures, and reference existing code as examples of patterns to follow.
...
BRANCH_NAME="codex/implement-$(date +%Y%m%d-%H%M%S)"
...
```bash
IMPL_OUT=$(mktemp /tmp/codex-impl-XXXXXX.txt)
```
...
8. **Launch Codex.** Check if running inside tmux and branch accordingly:
...
tmux split-window -h -c "$(pwd)" -d "bash /tmp/codex-impl-run.sh '$(pwd)' '${IMPL_OUT}'"
```
...
10. **Review changes.** Examine what Codex actually did:
...
11. **Run quality review.** Invoke `/codex-review` to get an independent code review of the implementation. Follow the full codex-review process including active judgment and iteration.

12. **Handle review failures.** If codex-review identifies accepted issues that need code changes:
   ...
   Launch using the same tmux/non-tmux pattern from step 8. After fixes, re-run `/codex-review`. Repeat until review passes (score >= 8.0, 0 critical issues).

13. **Run verification.** If the plan specified verification commands (tests, build, lint), run them now.

   If verification fails, compile error details into `FIX_INSTRUCTIONS` and run Codex again using the same pattern as step 12. Re-verify after fixes. Repeat until verification passes.
