...
(
  ...
  if [ -n "$RIGHT_PANES" ]; then
    ...
    NEW_PANE=$(tmux split-window -v -t "$LAST_RIGHT" -c "$(pwd)" -d -P -F '#{pane_id}' "bash /tmp/codex-review-run.sh '$(pwd)' '${REVIEW_OUT}'")
  else
    NEW_PANE=$(tmux split-window -h -t 0 -c "$(pwd)" -d -P -F '#{pane_id}' "bash /tmp/codex-review-run.sh '$(pwd)' '${REVIEW_OUT}'")
  fi
